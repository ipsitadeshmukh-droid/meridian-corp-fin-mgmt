...
import { Action, HomeLeftNavItem, RuntimeFilterOp, HomepageModule, HostEvent, ContextMenuTriggerOptions, CustomActionTarget, CustomActionsPosition, ErrorDetailsTypes, EmbedErrorCodes, EmbedErrorSeverity, } from '../types';
...
describe('Unit test case for ts embed', () => {
    ...
    describe('error severity', () => {
        ...
        test('defaults to SEV3 when the raising site does not set one', () => {
            ...
            expect(errorHandler.mock.calls[0][0]).toEqual(expect.objectContaining({ severity: EmbedErrorSeverity.SEV3 }));
        });
        test('does not override a severity the raising site set', () => {
            ...
            expect(errorHandler.mock.calls[0][0]).toEqual(expect.objectContaining({ severity: EmbedErrorSeverity.SEV1 }));
        });
    });
    describe('V1Embed ', () => {
        beforeEach(() => {
    ...
    describe('validate preRender flow', () => {
        ...
        it('should skip syncing styles when the placeholder is detached', async () => {
            ...
            expect(logger.debug).toHaveBeenCalledWith('syncPreRenderStyle skipped: placeholder is detached');
...
describe('ShowPreRender with UpdateEmbedParams', () => {
    ...
    test('should carry the new config filters in UpdateEmbedParams', async () => {
        ...
        await executeAfterWait(() => {
            ...
            const runtimeFilterCalls = mockProcessTrigger.mock.calls.filter((call) => call[1] === HostEvent.UpdateRuntimeFilters);
    ...
    test('should trigger Navigate only after UpdateEmbedParams has settled', async () => {
        ...
        await executeAfterWait(() => {
            ...
            expect(eventTypes.indexOf(HostEvent.UpdateEmbedParams)).toBeLessThan(eventTypes.indexOf(HostEvent.Navigate));
        }, 300);
    });
    test('should mark the params empty when the config declares no filters', async () => {
        ...
        await executeAfterWait(() => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(expect.any(Object), HostEvent.UpdateEmbedParams, expect.any(String), expect.objectContaining({ runtimeFilterParams: '&' }), undefined);
            const runtimeFilterCalls = mockProcessTrigger.mock.calls.filter((call) => call[1] === HostEvent.UpdateRuntimeFilters);
            expect(runtimeFilterCalls).toHaveLength(0);
        });
    });
    ...
    test('should mark the params empty when no config in the chain has filters', async () => {
        ...
        embed2.showPreRender();
        await executeAfterWait(() => {
            const paramsCall = mockProcessTrigger.mock.calls.find((call) => call[1] === HostEvent.UpdateEmbedParams);
    ...
    test('should carry the new config runtime parameters in UpdateEmbedParams', async () => {
        ...
        await executeAfterWait(() => {
            ...
            const parameterCalls = mockProcessTrigger.mock.calls.filter((call) => call[1] === HostEvent.UpdateParameters);
    ...
    test('should mark the params empty when the config declares no parameters', async () => {
        ...
        await executeAfterWait(() => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(expect.any(Object), HostEvent.UpdateEmbedParams, expect.any(String), expect.objectContaining({ runtimeParameterParams: '&' }), undefined);
        });
    });
    ...
    test('should not strand callbacks when the container loads after the shown embed was hidden', async () => {
        ...
        embed1.createEmbedContainerHandler(EmbedEvent.EmbedListenerReady)();
        mockProcessTrigger.mockClear();
        mockProcessTrigger.mockResolvedValue({});
    ...
    describe('trigger typing contracts (compile-time)', () => {
        it('infers typed response for no-payload calls on the base trigger', () => {
            const searchEmbed = new SearchEmbed(getRootEl(), defaultViewConfig);
            const getTabs = () => searchEmbed.trigger(HostEvent.GetTabs);
            ...
            const reload = () => searchEmbed.trigger(HostEvent.Reload);
