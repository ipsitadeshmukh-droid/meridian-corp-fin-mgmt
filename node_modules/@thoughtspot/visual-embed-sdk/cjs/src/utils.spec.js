...
describe('deserializeParam', () => {
    ...
    test('parses a JSON-string array back to a real array (SCAL-334713)', () => {
        expect((0, utils_1.deserializeParam)('["4dd30af7-9ed7-4847-8f28-b65b44a841d8"]')).toEqual([
            '4dd30af7-9ed7-4847-8f28-b65b44a841d8',
        ]);
    });
    test('does not URI-decode strings — raw %xx content must survive', () => {
        expect((0, utils_1.deserializeParam)('%5Bcommit%20date%5D%5Brevenue%5D')).toBe('%5Bcommit%20date%5D%5Brevenue%5D');
    });
    test('parses JSON objects and booleans', () => {
        expect((0, utils_1.deserializeParam)('{"a":1}')).toEqual({ a: 1 });
        expect((0, utils_1.deserializeParam)('true')).toBe(true);
    });
    test('keeps numeric strings as strings, matching the app URL parser', () => {
        expect((0, utils_1.deserializeParam)('123')).toBe('123');
        expect((0, utils_1.deserializeParam)('1.5')).toBe('1.5');
    });
    test('keeps plain non-JSON strings unchanged', () => {
        expect((0, utils_1.deserializeParam)('local-host')).toBe('local-host');
        expect((0, utils_1.deserializeParam)('AuthServerCookieless')).toBe('AuthServerCookieless');
    });
    test('keeps a malformed percent-sequence string as-is', () => {
        expect((0, utils_1.deserializeParam)('100% legit')).toBe('100% legit');
    });
    test('passes non-string values through untouched', () => {
        const arr = ['a'];
        expect((0, utils_1.deserializeParam)(arr)).toBe(arr);
        expect((0, utils_1.deserializeParam)(true)).toBe(true);
        expect((0, utils_1.deserializeParam)(42)).toBe(42);
        expect((0, utils_1.deserializeParam)(undefined)).toBeUndefined();
    });
});
...
describe('isValidCssMargin', () => {
    ...
    it('should accept DEFAULT_LAZY_LOADING_MARGIN', () => {
        expect((0, utils_1.isValidCssMargin)(config_1.DEFAULT_LAZY_LOADING_MARGIN)).toBe(true);
    });
...
describe('calculateElementCenter', () => {
    const setViewport = (scrollY, innerHeight) => {
        Object.defineProperty(window, 'scrollY', { value: scrollY, configurable: true });
        Object.defineProperty(window, 'innerHeight', { value: innerHeight, configurable: true });
    };
    const mockElement = (offsetTop, offsetHeight) => ({
    ...
    test('centers on the element when it is shorter than the viewport', () => {
        ...
        const result = (0, utils_1.calculateElementCenter)(mockElement(0, 400));
        expect(result).toEqual({
            iframeCenter: 200,
            iframeScrolled: 0,
            iframeHeight: 400,
    ...
    test('centers on the visible slice when the element is taller than the viewport', () => {
        ...
        const result = (0, utils_1.calculateElementCenter)(mockElement(0, 2000));
    ...
    test('measures only the on-screen part when the element starts below the fold', () => {
        ...
        const result = (0, utils_1.calculateElementCenter)(mockElement(800, 400));
    ...
    test('never reports more visible height than the element has', () => {
        ...
        const result = (0, utils_1.calculateElementCenter)(mockElement(200, 100));
