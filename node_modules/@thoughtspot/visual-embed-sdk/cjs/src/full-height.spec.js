...
const full_height_1 = require("./full-height");
...
describe('FullHeightController', () => {
     ...
     */
    const rectOf = (top, bottom, left = 0, right = 500) => ({
        top,
        bottom,
        left,
        right,
    ...
    const setViewport = (scrollY, innerHeight, innerWidth = 1024) => {
        Object.defineProperty(window, 'scrollY', { value: scrollY, configurable: true });
        Object.defineProperty(window, 'innerHeight', { value: innerHeight, configurable: true });
        Object.defineProperty(window, 'innerWidth', { value: innerWidth, configurable: true });
    };
    const createControllerFor = (viewConfig) => createController(viewConfig);
    const createController = (viewConfig, options = {}) => {
        handlers = new Map();
        ...
        return new full_height_1.FullHeightController(viewConfig, host);
    };
    /**
     ...
     */
    const queryParamsFor = (viewConfig) => {
        const params = {};
     ...
     */
    const mountInScrollContainer = (viewConfig, options = {}) => {
        const observe = jest.fn();
        const disconnect = jest.fn();
        let resizeCallback;
        const resizeObserverCtor = jest.fn((callback) => {
    ...
    const visibleCoordinates = () => {
        ...
        handlers.get(types_1.EmbedEvent.RequestVisibleEmbedCoordinates)({}, responder);
    ...
    describe('registerEventHandlers', () => {
        it('registers every full-height handler when fullHeight is enabled', () => {
            ...
            expect([...handlers.keys()]).toEqual([
                types_1.EmbedEvent.RouteChange,
                types_1.EmbedEvent.EmbedHeight,
                types_1.EmbedEvent.EmbedIframeCenter,
                types_1.EmbedEvent.RequestVisibleEmbedCoordinates,
            ]);
        });
    ...
    describe('viewConfig', () => {
        it('reads back the lazy-loading settings defaulted for the host app', () => {
            ...
            expect(controller.viewConfig).toEqual({
                ...
                lazyLoadingMargin: config_1.DEFAULT_LAZY_LOADING_MARGIN,
            });
        });
        ...
        it('re-resolves the defaults when the config is replaced', () => {
            ...
            const next = { fullHeight: true, minimumHeight: 900 };
            controller.viewConfig = next;
            expect(controller.minimumHeight).toBe(900);
            expect(controller.viewConfig.lazyLoadingMargin).toBe(config_1.DEFAULT_LAZY_LOADING_MARGIN);
        ...
        it('feeds the query params from the replaced config', () => {
            ...
            expect(params).toEqual({ [types_1.Param.fullHeight]: true });
        });
    });
    describe('lazy loading defaults', () => {
        it('turns lazy loading on for a full-height embed', () => {
            expect(queryParamsFor({ fullHeight: true })).toEqual({
                [types_1.Param.fullHeight]: true,
                [types_1.Param.IsLazyLoadingForEmbedEnabled]: true,
                [types_1.Param.RootMarginForLazyLoad]: config_1.DEFAULT_LAZY_LOADING_MARGIN,
            });
        });
        ...
        it('leaves the view config the host app passed in untouched', () => {
            ...
            const viewConfig = { fullHeight: true };
            const controller = new full_height_1.FullHeightController(viewConfig, {
                getIframe: () => iFrame,
                setFrameHeight: jest.fn(),
                on: jest.fn(),
                trigger: jest.fn(),
        ...
        it('leaves an explicit opt-out alone', () => {
            expect(queryParamsFor({
                ...
                lazyLoadingForFullHeight: false,
                lazyLoadingMargin: '0px',
            })).toEqual({ [types_1.Param.fullHeight]: true });
        });
        it('honours an explicit container opt-out', () => {
            const { controller, addContainerListener, resizeObserverCtor } = mountInScrollContainer({ fullHeight: true, enableScrollableContainerLazyLoading: false });
            controller.onRender();
        ...
        it('defaults only the values the host app left unset', () => {
            ...
            expect(params).toEqual({
                [types_1.Param.fullHeight]: true,
                [types_1.Param.IsLazyLoadingForEmbedEnabled]: true,
                [types_1.Param.RootMarginForLazyLoad]: '50px',
            });
            controller.onRender();
            expect(addContainerListener).not.toHaveBeenCalled();
        });
    ...
    describe('addQueryParams', () => {
        ...
        it('adds the lazy loading params, including a valid margin', () => {
            expect(queryParamsFor({
                ...
                [types_1.Param.fullHeight]: true,
                [types_1.Param.IsLazyLoadingForEmbedEnabled]: true,
                [types_1.Param.RootMarginForLazyLoad]: '100px 0px',
            });
        });
        it('accepts a four sided margin', () => {
            ...
            })[types_1.Param.RootMarginForLazyLoad]).toBe('10px 20px 30px 40px');
        });
        it('accepts a unitless zero margin', () => {
            expect(queryParamsFor({
                fullHeight: true,
                lazyLoadingMargin: '0',
            })[types_1.Param.RootMarginForLazyLoad]).toBe('0');
        });
        ...
        it('drops an invalid lazy loading margin', () => {
            ...
            const loggerError = jest.spyOn(logger_1.logger, 'error').mockImplementation(jest.fn());
            const params = queryParamsFor({
                fullHeight: true,
                lazyLoadingForFullHeight: true,
                lazyLoadingMargin: 'not-a-margin',
            });
            expect(params[types_1.Param.RootMarginForLazyLoad]).toBeUndefined();
    ...
    describe('EmbedHeight', () => {
        ...
        it('leaves the height alone when the app reports a non-numeric height', () => {
            createController({ fullHeight: true });
            handlers.get(types_1.EmbedEvent.EmbedHeight)(embedHeight('tall'));
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
        it('leaves the height alone when the payload carries no height', () => {
            createController({ fullHeight: true });
            handlers.get(types_1.EmbedEvent.EmbedHeight)({});
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
        it('survives a missing payload', () => {
            createController({ fullHeight: true });
            expect(() => handlers.get(types_1.EmbedEvent.EmbedHeight)(undefined)).not.toThrow();
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
        it('still pushes the visible coordinates for an unusable height', () => {
            ...
            handlers.get(types_1.EmbedEvent.EmbedHeight)(embedHeight('tall'));
            expect(host.trigger).toHaveBeenCalledWith(types_1.HostEvent.VisibleEmbedCoordinates, expect.objectContaining({ top: expect.any(Number) }));
        });
    ...
    describe('RouteChange', () => {
        const routeChange = (currentPath) => ({ data: { currentPath } });
        it('leaves the height alone while navigating within a Liveboard', () => {
            createController({ fullHeight: true });
            handlers.get(types_1.EmbedEvent.RouteChange)(routeChange('/embed/viz/abc'));
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
        it.each([
            '/pinboard/abc',
            ...
            createController({ fullHeight: true });
            handlers.get(types_1.EmbedEvent.RouteChange)(routeChange(currentPath));
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
        it('leaves the height alone on a bare Liveboard route', () => {
            createController({ fullHeight: true });
            handlers.get(types_1.EmbedEvent.RouteChange)(routeChange('/liveboard/'));
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
        it('resets to frameParams.height when leaving the Liveboard routes', () => {
            createController({ fullHeight: true, frameParams: { height: 640 } });
            handlers.get(types_1.EmbedEvent.RouteChange)(routeChange('/some/other/path/'));
        ...
        it('resets when a Liveboard route only appears part way into the path', () => {
            ...
            createController({ fullHeight: true, minimumHeight: 800 });
            handlers.get(types_1.EmbedEvent.RouteChange)(routeChange('/app/embed/viz/abc'));
            expect(host.setFrameHeight).toHaveBeenCalledWith(800);
        });
        ...
        it('leaves the height alone when the payload carries no path', () => {
            createController({ fullHeight: true });
            handlers.get(types_1.EmbedEvent.RouteChange)({ data: {} });
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
        it('survives a missing payload', () => {
            createController({ fullHeight: true });
            expect(() => handlers.get(types_1.EmbedEvent.RouteChange)(undefined)).not.toThrow();
            expect(host.setFrameHeight).not.toHaveBeenCalled();
        });
    ...
    describe('coordinate requests', () => {
        ...
        it('responds to EmbedIframeCenter with the center of the visible region', () => {
            createController({ fullHeight: true });
            const responder = jest.fn();
            handlers.get(types_1.EmbedEvent.EmbedIframeCenter)({}, responder);
            expect(responder).toHaveBeenCalledWith({
        ...
        it('logs the RequestVisibleEmbedCoordinates request', () => {
            const loggerInfo = jest.spyOn(logger_1.logger, 'info').mockImplementation(jest.fn());
            createController({ fullHeight: true });
            const payload = { type: types_1.EmbedEvent.RequestVisibleEmbedCoordinates };
            handlers.get(types_1.EmbedEvent.RequestVisibleEmbedCoordinates)(payload, jest.fn());
            expect(loggerInfo).toHaveBeenCalledWith('Sending RequestVisibleEmbedCoordinates', payload);
        });
        ...
        it('survives an app that asks for coordinates without a responder', () => {
            createController({ fullHeight: true });
            expect(() => {
                handlers.get(types_1.EmbedEvent.RequestVisibleEmbedCoordinates)({});
                handlers.get(types_1.EmbedEvent.EmbedIframeCenter)({});
            }).not.toThrow();
        });
    ...
    describe('lazy load listeners', () => {
        ...
        it('observes the scrollable ancestors when container lazy loading is on', () => {
            const { controller, addContainerListener, removeContainerListener, observe, disconnect, } = mountInScrollContainer({
                fullHeight: true,
                lazyLoadingForFullHeight: true,
                enableScrollableContainerLazyLoading: true,
            });
        ...
        it('still tracks the containers in an environment without ResizeObserver', () => {
            const { controller, addContainerListener } = mountInScrollContainer({ fullHeight: true }, { withResizeObserver: false });
            expect(() => controller.onRender()).not.toThrow();
            expect(addContainerListener).toHaveBeenCalledWith('scroll', expect.any(Function));
            expect(() => controller.destroy()).not.toThrow();
        });
        it('drops the previous containers when the embed re-renders', () => {
            const { controller, removeContainerListener, disconnect, } = mountInScrollContainer({ fullHeight: true });
            controller.onRender();
            controller.onRender();
            expect(removeContainerListener).toHaveBeenCalledWith('scroll', expect.any(Function));
...
//# sourceMappingURL=full-height.spec.js.map