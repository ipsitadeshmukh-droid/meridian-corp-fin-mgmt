...
describe('Search embed tests', () => {
    ...
    test('getUpdateEmbedParamsObject sends dataSources as a real ARRAY, not the URL-encoded string', async () => {
        ...
        const params = await searchEmbed.getUpdateEmbedParamsObject();
        ...
        const singleSourceEmbed = new search_1.SearchEmbed((0, test_utils_1.getRootEl)(), {
            ...defaultViewConfig,
            dataSource: '4dd30af7-9ed7-4847-8f28-b65b44a841d8',
        });
        const singleParams = await singleSourceEmbed.getUpdateEmbedParamsObject();
    ...
    test('should set visibility and disable state for Chart Settings V2 menu items', async () => {
        expect(types_1.Action.ChartTypeSettings).toBe('CHART_TYPE');
        expect(types_1.Action.LayoutSettings).toBe('LAYOUT');
        expect(types_1.Action.ColumnSettings).toBe('COLUMN');
        expect(types_1.Action.AxisSettings).toBe('AXIS');
        expect(types_1.Action.DataLabelSettings).toBe('DATA_LABEL');
        expect(types_1.Action.TooltipSettings).toBe('TOOLTIP');
        expect(types_1.Action.LegendSettings).toBe('LEGEND');
        expect(types_1.Action.DisplaySettings).toBe('DISPLAY');
        expect(types_1.Action.QuerySettings).toBe('QUERY_DETAILS');
        expect(types_1.Action.CustomSettings).toBe('CUSTOM_ACTION');
        expect(types_1.Action.MuzeAiSettings).toBe('MUZE_AI');
        expect(types_1.Action.RAnalysisSettings).toBe('R_ANALYSIS');
        const chartSettingsActions = [
            types_1.Action.ChartTypeSettings,
            types_1.Action.LayoutSettings,
            types_1.Action.ColumnSettings,
            types_1.Action.AxisSettings,
            types_1.Action.DataLabelSettings,
            types_1.Action.TooltipSettings,
            types_1.Action.LegendSettings,
            types_1.Action.DisplaySettings,
            types_1.Action.QuerySettings,
            types_1.Action.CustomSettings,
            types_1.Action.MuzeAiSettings,
            types_1.Action.RAnalysisSettings,
        ];
        const searchEmbed = new search_1.SearchEmbed((0, test_utils_1.getRootEl)(), {
        ...
        const disableActionUrl = (0, test_utils_1.fixedEncodeURI)(JSON.stringify(chartSettingsActions));
        await (0, test_utils_1.executeAfterWait)(() => {
            (0, test_utils_1.expectUrlMatchesWithParams)((0, test_utils_1.getIFrameSrc)(), `http://${thoughtSpotHost}/v2/?${test_utils_1.defaultParamsWithoutHiddenActions}&disableAction=${disableActionUrl}&disableHint=Access%20denied&hideAction=${hideActionUrl}&dataSourceMode=expand&useLastSelectedSources=false${prefixParams}#/embed/saved-answer/${answerId}`);
        });
    });
    ...
    test('Should add showAnswerEditPanel flag set to false to the iframe src when hideAnswerEditPanel is true', async () => {
        ...
        searchEmbed.render();
        await (0, test_utils_1.executeAfterWait)(() => {
            (0, test_utils_1.expectUrlMatchesWithParams)((0, test_utils_1.getIFrameSrc)(), `http://${thoughtSpotHost}/v2/?${defaultParamsWithHiddenActions}&showAnswerEditPanel=false&dataSourceMode=expand&useLastSelectedSources=false${prefixParams}#/embed/answer`);
        });
    });
    test('Should add showAnswerEditPanel flag set to true to the iframe src when hideAnswerEditPanel is false', async () => {
        ...
        searchEmbed.render();
        await (0, test_utils_1.executeAfterWait)(() => {
            (0, test_utils_1.expectUrlMatchesWithParams)((0, test_utils_1.getIFrameSrc)(), `http://${thoughtSpotHost}/v2/?${defaultParamsWithHiddenActions}&showAnswerEditPanel=true&dataSourceMode=expand&useLastSelectedSources=false${prefixParams}#/embed/answer`);
        });
    });
