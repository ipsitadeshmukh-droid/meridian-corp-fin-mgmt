...
describe('Unit test case for ts embed', () => {
    ...
    describe('Vaidate iframe properties', () => {
        ...
        test('triggerUIPassThrough with params', async () => {
            ...
            await (0, test_utils_1.executeAfterWait)(async () => {
                ...
                await searchEmbed.triggerUIPassThrough(ui_passthrough_contracts_1.UIPassthroughEvent.PinAnswerToLiveboard, payload);
                expect(mockProcessTrigger).toHaveBeenCalledWith((0, test_utils_1.getIFrameEl)(), types_1.HostEvent.UIPassthrough, 'http://tshost', {
                    parameters: payload,
                    type: ui_passthrough_contracts_1.UIPassthroughEvent.PinAnswerToLiveboard,
                }, undefined);
            });
        });
    ...
    describe('when thoughtSpotHost is empty', () => {
        ...
        test('Error should be true', async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith({
                ...
                severity: types_1.EmbedErrorSeverity.SEV1,
        ...
        test('a missing init is reported to the host as SEV1', async () => {
            ...
            const tsEmbed = new index_1.SearchEmbed((0, test_utils_1.getRootEl)(), {});
            tsEmbed.on(index_1.EmbedEvent.Error, errorHandler);
            await tsEmbed.render();
    ...
    describe('error severity', () => {
        ...
        test('defaults to SEV3 when the raising site does not set one', () => {
            ...
            expect(errorHandler.mock.calls[0][0]).toEqual(expect.objectContaining({ severity: types_1.EmbedErrorSeverity.SEV3 }));
        });
        test('does not override a severity the raising site set', () => {
            ...
            expect(errorHandler.mock.calls[0][0]).toEqual(expect.objectContaining({ severity: types_1.EmbedErrorSeverity.SEV1 }));
        });
    });
    describe('V1Embed ', () => {
        beforeEach(() => {
    ...
    describe('validate preRender flow', () => {
        ...
        it('should skip syncing styles when the placeholder is detached', async () => {
            (0, test_utils_1.createRootEleForEmbed)();
            jest.spyOn(logger_1.logger, 'debug').mockImplementation(jest.fn());
            const libEmbed = new index_1.LiveboardEmbed('#tsEmbedDiv', {
                liveboardId: 'myLiveboardId',
            ...
            logger_1.logger.debug.mockClear();
            ...
            expect(logger_1.logger.debug).toHaveBeenCalledWith('syncPreRenderStyle skipped: placeholder is detached');
            ...
            libEmbed.destroy();
            logger_1.logger.debug.mockRestore();
        });
        describe('preRenderContainer', () => {
            ...
            it('should restore the original container position on destroy', async () => {
                ...
                expect(libEmbed.preRenderContainerEl).toBe(document.body);
                customContainer.remove();
            });
            ...
            it('should position the newly resolved container after a remount, not the stale one', async () => {
                ...
                expect(libEmbed.preRenderContainerEl).toBe(newContainer);
            ...
            describe('connectPreRendered container init', () => {
                ...
                it('should resolve preRenderContainerEl when connecting to an existing pre-render', async () => {
                    ...
                    await firstEmbed.preRender();
                    await (0, test_utils_1.waitFor)(() => !!(0, test_utils_1.getIFrameEl)());
                    const connectingEmbed = new index_1.LiveboardEmbed('#tsEmbedDiv', {
                        preRenderId: 'connect-init-default',
                        liveboardId: 'myLiveboardId',
                    });
                    await connectingEmbed.showPreRender();
                    expect(connectingEmbed.preRenderContainerEl).not.toBeNull();
                    firstEmbed.destroy();
                });
                it('should fall back to document.body when no preRenderContainer is configured on the connecting embed', async () => {
                    ...
                    await connectingEmbed.showPreRender();
                    expect(connectingEmbed.preRenderContainerEl).toBe(document.body);
                    firstEmbed.destroy();
                });
                it('should resolve a custom preRenderContainer on the connecting embed', async () => {
                    ...
                    await connectingEmbed.showPreRender();
                    expect(connectingEmbed.preRenderContainerEl).toBe(customContainer);
                    firstEmbed.destroy();
                    customContainer.remove();
                });
                ...
                it('should inherit the container from the pre-rendering embed instead of re-resolving it', async () => {
                    (0, test_utils_1.createRootEleForEmbed)();
                    const customContainer = document.createElement('div');
                    customContainer.id = 'connect-inherit-container';
                    document.body.appendChild(customContainer);
                    const firstEmbed = new index_1.LiveboardEmbed('#tsEmbedDiv', {
                    ...
                    await firstEmbed.preRender();
                    await (0, test_utils_1.waitFor)(() => !!(0, test_utils_1.getIFrameEl)());
                    // No preRenderContainer here: the container belongs to the
                    // PreRender component and is adopted as-is.
                    const connectingEmbed = new index_1.LiveboardEmbed('#tsEmbedDiv', {
                        preRenderId: 'connect-no-re-resolve',
                        liveboardId: 'myLiveboardId',
                    });
                    const resolveSpy = jest.spyOn(connectingEmbed, 'resolvePreRenderContainerTarget');
                    connectingEmbed.connectPreRendered();
                    expect(connectingEmbed.preRenderContainerEl).toBe(customContainer);
                    expect(resolveSpy).not.toHaveBeenCalled();
                    firstEmbed.destroy();
                    customContainer.remove();
                });
                it('should warn and ignore a preRenderContainer passed on the connecting embed', async () => {
                    (0, test_utils_1.createRootEleForEmbed)();
                    const ownerContainer = document.createElement('div');
                    ownerContainer.id = 'connect-owner-container';
                    const otherContainer = document.createElement('div');
                    otherContainer.id = 'connect-other-container';
                    document.body.appendChild(ownerContainer);
                    document.body.appendChild(otherContainer);
                    const warnSpy = jest.spyOn(logger_1.logger, 'warn').mockImplementation(() => undefined);
                    const firstEmbed = new index_1.LiveboardEmbed('#tsEmbedDiv', {
                        preRenderId: 'connect-container-warn',
                        liveboardId: 'myLiveboardId',
                        preRenderContainer: ownerContainer,
                    });
                    await firstEmbed.preRender();
                    await (0, test_utils_1.waitFor)(() => !!(0, test_utils_1.getIFrameEl)());
                    const connectingEmbed = new index_1.LiveboardEmbed('#tsEmbedDiv', {
                        preRenderId: 'connect-container-warn',
                        liveboardId: 'myLiveboardId',
                        preRenderContainer: otherContainer,
                    });
                    connectingEmbed.connectPreRendered();
                    expect(connectingEmbed.preRenderContainerEl).toBe(ownerContainer);
                    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('preRenderContainer is applied only by'));
                    warnSpy.mockRestore();
                    firstEmbed.destroy();
                ...
                it('should not warn when the connecting embed passes the same container element', async () => {
                    ...
                    connectingEmbed.connectPreRendered();
                    expect(connectingEmbed.preRenderContainerEl).toBe(customContainer);
                    expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining('preRenderContainer is applied only by'));
                    warnSpy.mockRestore();
                    firstEmbed.destroy();
                    customContainer.remove();
                });
                ...
                it('should keep its own container when the same instance owns the pre-render', async () => {
                    ...
                    libEmbed.connectPreRendered();
                    expect(libEmbed.preRenderContainerEl).toBe(customContainer);
                    expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining('preRenderContainer is applied only by'));
                    warnSpy.mockRestore();
                    libEmbed.destroy();
                    customContainer.remove();
                ...
                it('should stay on document.body when the wrapper has no owning embed', async () => {
                    ...
                    const wrapper = document.getElementById(firstEmbed.getPreRenderIds().wrapper);
                    delete wrapper.__tsEmbed;
                    const connectingEmbed = new index_1.LiveboardEmbed('#tsEmbedDiv', {
                        preRenderId: 'connect-unstamped',
                        liveboardId: 'myLiveboardId',
                    });
    ...
    describe('Embed Container Loading', () => {
        ...
        test('should return getPreRenderObj and log if same object', () => {
            ...
            const loggerSpy = jest.spyOn(logger_1.logger, 'debug');
...
describe('Trigger method edge cases', () => {
    ...
    test('isValidationError catch: handleError called with embedErrorDetails and error is rethrown', async () => {
        ...
        mockProcessTrigger.mockResolvedValueOnce([
            { value: { keys: Object.values(ui_passthrough_contracts_1.UIPassthroughEvent) } },
        ]);
...
describe('ShowPreRender with UpdateEmbedParams', () => {
    ...
    test('should carry the new config filters in UpdateEmbedParams', async () => {
        ...
        await (0, test_utils_1.executeAfterWait)(() => {
            ...
            const runtimeFilterCalls = mockProcessTrigger.mock.calls.filter((call) => call[1] === types_1.HostEvent.UpdateRuntimeFilters);
    ...
    test('should trigger Navigate only after UpdateEmbedParams has settled', async () => {
        ...
        // and Navigate is not.
        await (0, test_utils_1.executeAfterWait)(() => {
            const eventTypes = mockProcessTrigger.mock.calls.map((call) => call[1]);
            expect(eventTypes).toContain(types_1.HostEvent.UpdateEmbedParams);
            expect(eventTypes).not.toContain(types_1.HostEvent.Navigate);
        }, 50);
        await (0, test_utils_1.executeAfterWait)(() => {
            const eventTypes = mockProcessTrigger.mock.calls.map((call) => call[1]);
            ...
            expect(eventTypes).toContain(types_1.HostEvent.Navigate);
            expect(eventTypes.indexOf(types_1.HostEvent.UpdateEmbedParams)).toBeLessThan(eventTypes.indexOf(types_1.HostEvent.Navigate));
        }, 300);
    });
    test('should mark the params empty when the config declares no filters', async () => {
        ...
        await (0, test_utils_1.executeAfterWait)(() => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(expect.any(Object), types_1.HostEvent.UpdateEmbedParams, expect.any(String), expect.objectContaining({ runtimeFilterParams: '&' }), undefined);
            const runtimeFilterCalls = mockProcessTrigger.mock.calls.filter((call) => call[1] === types_1.HostEvent.UpdateRuntimeFilters);
            expect(runtimeFilterCalls).toHaveLength(0);
        });
    });
    ...
    test('should mark the params empty when no config in the chain has filters', async () => {
        ...
        embed2.showPreRender();
        await (0, test_utils_1.executeAfterWait)(() => {
            const paramsCall = mockProcessTrigger.mock.calls.find((call) => call[1] === types_1.HostEvent.UpdateEmbedParams);
            expect(paramsCall === null || paramsCall === void 0 ? void 0 : paramsCall[3]).not.toHaveProperty('runtimeFilters');
            expect(paramsCall === null || paramsCall === void 0 ? void 0 : paramsCall[3].runtimeFilterParams).toBe('&');
        });
    });
    test('should state each embed filters as the pre-render is handed along', async () => {
        ...
        // embed before it.
        const embed3 = new index_1.LiveboardEmbed('#tsEmbedDiv', {
            preRenderId: 'params-chain',
    ...
    test('should carry the new config runtime parameters in UpdateEmbedParams', async () => {
        ...
        await (0, test_utils_1.executeAfterWait)(() => {
            ...
            const parameterCalls = mockProcessTrigger.mock.calls.filter((call) => call[1] === types_1.HostEvent.UpdateParameters);
    ...
    test('should mark the params empty when the config declares no parameters', async () => {
        ...
        await (0, test_utils_1.executeAfterWait)(() => {
            ...
            expect(mockProcessTrigger).toHaveBeenCalledWith(expect.any(Object), types_1.HostEvent.UpdateEmbedParams, expect.any(String), expect.objectContaining({ runtimeParameterParams: '&' }), undefined);
        });
    });
    ...
    test('should hand the pre-render over to the embed being shown', async () => {
        ...
        expect(embed2.getPreRenderObj()).toBe(embed2);
        expect(embed1.getPreRenderObj()).toBe(embed2);
    });
    test('should not strand callbacks when the container loads after the shown embed was hidden', async () => {
        ...
        embed1.createEmbedContainerHandler(index_1.EmbedEvent.EmbedListenerReady)();
        mockProcessTrigger.mockClear();
        mockProcessTrigger.mockResolvedValue({});
    ...
    test('should not reconcile runtime filters unless the view config opts in', async () => {
        ...
        await (0, test_utils_1.executeAfterWait)(() => {
            const eventTypes = mockProcessTrigger.mock.calls.map((call) => call[1]);
            expect(eventTypes).toContain(types_1.HostEvent.UpdateEmbedParams);
            expect(eventTypes).not.toContain(types_1.HostEvent.UpdateRuntimeFilters);
        });
    });
    ...
    describe('trigger typing contracts (compile-time)', () => {
        it('infers typed response for no-payload calls on the base trigger', () => {
            const searchEmbed = new index_1.SearchEmbed((0, test_utils_1.getRootEl)(), defaultViewConfig);
            const getTabs = () => searchEmbed.trigger(types_1.HostEvent.GetTabs);
            ...
            const reload = () => searchEmbed.trigger(types_1.HostEvent.Reload);
