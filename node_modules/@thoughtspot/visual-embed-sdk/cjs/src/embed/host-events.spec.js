...
describe('HostEvent.OpenParameter', () => {
    test('postMessage type is openParameter (camelCase)', async () => {
        ...
        const { lb, iframe } = await renderLiveboard();
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(index_1.HostEvent.OpenParameter, { parameter: { parameterId: 'p1' } });
            expect(iframe.contentWindow.postMessage).toHaveBeenCalledWith(expect.objectContaining({ type: 'openParameter' }), thoughtSpotHost, expect.anything());
        });
    });
    test('accepts a payload with only parameterName', async () => {
        ...
        const { lb, iframe } = await renderLiveboard();
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(index_1.HostEvent.OpenParameter, { parameter: { parameterName: 'Param 1' } });
            expect(iframe.contentWindow.postMessage).toHaveBeenCalledWith(expect.objectContaining({ type: 'openParameter' }), thoughtSpotHost, expect.anything());
        });
    });
});
...
describe('HostEvent.OpenAddFilterModal', () => {
    test('postMessage type is openAddFilterModal (camelCase, no payload)', async () => {
        ...
        const { lb, iframe } = await renderLiveboard();
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(index_1.HostEvent.OpenAddFilterModal);
            expect(iframe.contentWindow.postMessage).toHaveBeenCalledWith(expect.objectContaining({ type: 'openAddFilterModal' }), thoughtSpotHost, expect.anything());
        });
    });
    test('returns null when !isRendered', async () => {
        const lb = unrenderedLiveboard();
        const result = await lb.trigger(index_1.HostEvent.OpenAddFilterModal);
        expect(result).toBeNull();
    });
});
// ---------------------------------------------------------------------------
...
describe('HostEvent.OpenAddParameterModal', () => {
    test('postMessage type is openAddParameterModal (camelCase, no payload)', async () => {
        ...
        const { lb, iframe } = await renderLiveboard();
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(index_1.HostEvent.OpenAddParameterModal);
            expect(iframe.contentWindow.postMessage).toHaveBeenCalledWith(expect.objectContaining({ type: 'openAddParameterModal' }), thoughtSpotHost, expect.anything());
        });
    });
    test('returns null when !isRendered', async () => {
        const lb = unrenderedLiveboard();
        const result = await lb.trigger(index_1.HostEvent.OpenAddParameterModal);
        expect(result).toBeNull();
    });
});
// ---------------------------------------------------------------------------
...
describe('HostEvent.SetVisibleVizs — array payload vizId injection', () => {
    test('vizId is added as a non-indexed property on the array — array.length unchanged', async () => {
        ...
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(index_1.HostEvent.SetVisibleVizs, arrayPayload);
            const call = iframe.contentWindow.postMessage.mock.calls[0];
            const sentData = call[0].data;
            expect(sentData.vizId).toBe(vizId);
            expect(sentData.length).toBe(2);
    ...
    test('returns null and calls handleError when !isRendered', async () => {
        const lb = unrenderedLiveboard();
        const result = await lb.trigger(index_1.HostEvent.SetVisibleVizs, ['viz1']);
        expect(result).toBeNull();
        expect(lb.handleError).toHaveBeenCalledWith(expect.objectContaining({ code: index_1.EmbedErrorCodes.RENDER_NOT_CALLED }));
    });
});
// ---------------------------------------------------------------------------
...
describe('HostEvent.UpdateRuntimeFilters — AppEmbed path', () => {
    async function renderApp() {
        const app = new index_1.AppEmbed((0, test_utils_1.getRootEl)(), {
            frameParams: { width: '100%', height: '100%' },
        });
        await app.render();
        const iframe = (0, test_utils_1.getIFrameEl)();
    ...
    test('AppEmbed sends payload directly without vizId injection', async () => {
        ...
        const filters = [{ columnName: 'col', operator: 'EQ', values: ['v1'] }];
        await (0, test_utils_1.executeAfterWait)(() => {
            app.trigger(index_1.HostEvent.UpdateRuntimeFilters, filters);
            const call = iframe.contentWindow.postMessage.mock.calls[0];
            const sentData = call[0].data;
...
describe('HostEvent.Present — vizId injection on LiveboardEmbed', () => {
    test('viewConfig.vizId is injected into the Present payload', async () => {
        ...
        const { lb, iframe } = await renderLiveboard({ vizId });
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(index_1.HostEvent.Present, {});
            expect(iframe.contentWindow.postMessage).toHaveBeenCalledWith(expect.objectContaining({
                data: expect.objectContaining({ vizId }),
            }), thoughtSpotHost, expect.anything());
        });
...
describe('HostEvent.DownloadAsCsv — additional cases', () => {
    test('viewConfig.vizId overwrites an explicit payload vizId', async () => {
        ...
        const { lb, iframe } = await renderLiveboard({ vizId });
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(index_1.HostEvent.DownloadAsCsv, { vizId: 'explicit-id' });
            const call = iframe.contentWindow.postMessage.mock.calls[0];
            const sentData = call[0].data;
    ...
    test('returns null and calls handleError when !isRendered', async () => {
        const lb = unrenderedLiveboard();
        const result = await lb.trigger(index_1.HostEvent.DownloadAsCsv, {});
        expect(result).toBeNull();
        expect(lb.handleError).toHaveBeenCalledWith(expect.objectContaining({ code: index_1.EmbedErrorCodes.RENDER_NOT_CALLED }));
    });
...
describe('Spotter share-conversation HostEvents', () => {
    const shareEvents = [
        ['ShareSpotterConversation', index_1.HostEvent.ShareSpotterConversation, 'ShareSpotterConversation', { conversationId: 'abc' }],
        ['CloseSpotterShareConversation', index_1.HostEvent.CloseSpotterShareConversation, 'CloseSpotterShareConversation', {}],
        ['ExitSpotterSharedConversation', index_1.HostEvent.ExitSpotterSharedConversation, 'ExitSpotterSharedConversation', {}],
    ];
    test.each(shareEvents)('%s: postMessage type is the matching string value', async (name, event, typeString, payload) => {
        (0, test_utils_1.mockMessageChannel)();
        const { lb, iframe } = await renderLiveboard();
        await (0, test_utils_1.executeAfterWait)(() => {
            lb.trigger(event, payload);
            expect(iframe.contentWindow.postMessage).toHaveBeenCalledWith(expect.objectContaining({ type: typeString }), thoughtSpotHost, expect.anything());
        });
    });
    test.each(shareEvents)('%s: returns null and calls handleError when !isRendered', async (name, event, typeString, payload) => {
        const lb = unrenderedLiveboard();
        const result = await lb.trigger(event, payload);
        expect(result).toBeNull();
        expect(lb.handleError).toHaveBeenCalledWith(expect.objectContaining({ code: index_1.EmbedErrorCodes.RENDER_NOT_CALLED }));
    });
});
